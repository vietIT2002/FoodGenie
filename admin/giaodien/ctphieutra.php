...
if (!empty($_SESSION['nguoidung'])) {
    ...
    $ctphieutra = mysqli_query($con, "SELECT `id_phieutra`, `id_sp`, `ctphieutra`.`so_luong`,`sanpham`.`id`,`ten_sp`,`don_gia`  FROM `ctphieutra`,`sanpham` WHERE `id_sp`=`sanpham`.`id` AND `id_phieutra`=" .$_GET['id']. " ORDER BY `ctphieutra`.`id_phieutra` ASC LIMIT " . $item_per_page . " OFFSET " . $offset);
    mysqli_close($con);
?>
